...
export class OutstandingRejectedComponent implements OnInit {
  ...
  constructor(private service: AddPurchaseorderService,private datePipe:DatePipe,
    ...
    this.getLimitedRejectedData(this.pharmacyId,0,20);
  ...
  getLimitedRejectedData(pharmacyId:number,start,end){
    this.loadRowData([], this.approvedGridOptions);
    
    this.service.getrejectedpurchaseordersbypharmacyWithLimit(pharmacyId,start,end).subscribe(
      res => {
        const data = res;
        for (var i = 0; i < res['result'].length; i++) {
         ...
         //  data['result'][i]['quantity'] = res['result'][i]['purchaseorderitems'] != null ? res['result'][i]['purchaseorderitems'][0]['totalQuantity'] : null;
           data['result'][i]['manuName'] = res['result'][i]['purchaseorderitems'].length  ? res['result'][i]['purchaseorderitems'][0]['itemsModel'] ? res['result'][i]['purchaseorderitems'][0]['itemsModel']['manufacturer']['name'] : null : null;
           //data['result'][i]['unitRate'] = res['result'][i]['purchaseorderitems'] != null ? res['result'][i]['purchaseorderitems'][0]['unitRate'] : null;
           data['result'][i]['packRate'] = res['result'][i]['purchaseorderitems'].length  ? res['result'][i]['purchaseorderitems'][0]['packRate'] : null;
         }
